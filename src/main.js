import imageURL_bilibili from './images/bilibili.png';
let dataWebsite = localStorage.getItem("dataWebsite") || null
let arrHashMap = JSON.parse(dataWebsite) || [
    { logo: "A", logoType: "text", url: "https://www.acfun.cn", domain: "acfun.cn" },
    { logo: imageURL_bilibili, logoType: "img", url: "https://www.bilibili.com", domain: "bilibili.com" }
]
...
let render = () => {
    arrHashMap.forEach((node) => {
        ...
        if (node.logoType === "img") {
            logo = `<img src="${node.logo}" alt="${node.domain}">`
